...
static void gsec_test_derive_data() {
   ...
   *  in Appendix A. **/
  size_t info_size = 10;
  const uint8_t info[info_size] = {0xf0, 0xf1, 0xf2, 0xf3, 0xf4,
                                   0xf5, 0xf6, 0xf7, 0xf8, 0xf9};
  size_t prk_size = 32;
  const uint8_t prk[prk_size] = {
  ...
  size_t out_size = 42;
  uint8_t* out_key =
      static_cast<uint8_t*>(gpr_zalloc(out_size * sizeof(uint8_t)));
  grpc_status_code derive_status = aes_gcm_derive_data(
      out_key, out_size, true, prk, prk_size, info, info_size);
  GPR_ASSERT(derive_status == GRPC_STATUS_OK);

  const uint8_t correct_out_key[82] = {
