...
const size_t kHandshakeClientOpNum = 4;
const char kS2AHandshakeServiceUrlForTesting[] = "test_url";
...
static bool validate_op(s2a_handshaker_client* client, const grpc_op* op,
  ...
  if (is_start) {
    ...
    ok &= (op->data.recv_initial_metadata.recv_initial_metadata == client->initial_metadata_for_testing());
    op++;
    GPR_ASSERT((size_t)(op - start_op) <= kHandshakerClientOpNum);
  }
  ok &= (op->op == GRPC_OP_SEND_MESSAGE);
  ok &= (op->data.send_message.send_message == client->send_buffer_for_testing());
  op++;
  GPR_ASSERT((size_t)(op - start_op) <= kHandshakerClientOpNum);
  ok &= (op->op == GRPC_OP_RECV_MESSAGE);
  ok &= (op->data.recv_message.recv_message == client->recv_buffer_for_testing());
  op++;
  GPR_ASSERT((size_t)(op - start_op) <= kHandshakerClientOpNum);
  return ok;
}
...
static grpc_call_error check_client_start_success(grpc_call* /*call*/,
  ...
  upb::Arena arena;
  s2a_handshaker_client* client = static_cast<s2a_handshaker_client*>(closure->cb_arg);
  GPR_ASSERT(client->closure_for_testing() == closure);
  s2a_SessionReq* request = s2a_deserialize_session_req(arena.ptr(), client->send_buffer_for_testing());
  GPR_ASSERT(s2a_SessionReq_has_client_start(request));
  s2a_ClientSessionStartReq* client_start = s2a_SessionReq_mutable_client_start(request);

  const upb_strview* application_protocols =
      s2a_ClientSessionStartReq_application_protocols(client_start, &application_protocols_size);
...
static grpc_call_error check_server_start_success(grpc_call* /*call*/,
  ...
  GPR_ASSERT(client->closure_for_testing() == closure);
  s2a_SessionReq* request = s2a_deserialize_session_req(arena.ptr(), client->send_buffer_for_testing());
  GPR_ASSERT(s2a_SessionReq_has_server_start(request));
  ...
  GPR_ASSERT(upb_strview_eql(in_bytes, upb_strview_makez(kS2AHandshakeClientTestOutFrame)));
...
static grpc_s2a_credentials_options* s2a_create_credentials_options(bool is_client) {
  grpc_s2a_credentials_options* options = grpc_s2a_credentials_options_create();
  options->set_handshaker_service_url(kS2AHandshakeServiceUrlForTesting);
  if (is_client) {
...
static s2a_handshaker_client_test_config* s2a_create_config() {
  s2a_handshaker_client_test_config* config = new s2a_handshaker_client_test_config();
  config->channel = grpc_insecure_channel_create(kS2AHandshakeServiceUrlForTesting, nullptr, nullptr);
  grpc_s2a_credentials_options* client_options = s2a_create_credentials_options(/* is_client=*/true);
  grpc_s2a_credentials_options* server_options = s2a_create_credentials_options(/*  is_client=*/false);
  tsi_result server_result = s2a_handshaker_client_create(
      /* handshaker=*/nullptr, config->channel, /* interested_parties=*/nullptr,
      server_options, grpc_slice_from_static_string(kS2AHandshakerClientTestTargetName),
  ...
  tsi_result client_result = s2a_handshaker_client_create(
      /* handshaker=*/nullptr, config->channel, /* interested_parties=*/nullptr,
      client_options, grpc_slice_from_static_string(kS2AHandshakerClientTestTargetName),
      /* grpc_cb=*/nullptr, /* cb=*/nullptr, /* user_data=*/nullptr,
  ...
  config->out_frame = grpc_slice_from_static_string(kS2AHandshakerClientTestOutFrame);
  return config;
}
...
static void s2a_schedule_request_success_test() {
  ...
  GPR_ASSERT(config->server->server_start(kTlsAes128GcmSha256, &(config->out_frame)) == TSI_OK);
