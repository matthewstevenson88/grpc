...
r.c * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
  ...
  switch (type) {
    ...
    case S2AResponseType::CLIENT_NEXT:
      ...
      s2a_SessionState_set_in_key(
          state, upb_strview_make(
                     reinterpret_cast<const char*>(
                         s2a_test_data::aes_128_gcm_traffic_secret.data()),
                     s2a_test_data::aes_128_gcm_traffic_secret.size()));
      ...
      s2a_SessionState_set_tls_version(state, s2a_TLS1_3);
      break;
...
static void on_client_next_success_cb(tsi_result status, void* user_data,
  ...
  /** Validate zero copy grpc protector. **/
  {
    grpc_core::ExecCtx exec_ctx;
    tsi_zero_copy_grpc_protector* protector = nullptr;
    grpc_channel* test_channel = new grpc_channel();
    s2a_tsi_handshaker_result_set_channel_for_testing(result, test_channel);
    GPR_ASSERT(tsi_handshaker_result_create_zero_copy_grpc_protector(
                   result, nullptr, &protector) == TSI_OK);
    s2a_tsi_handshaker_result_set_channel_for_testing(result, nullptr);
    delete test_channel;
...
static tsi_result mock_client_start(void* client) {
  GPR_ASSERT(client != nullptr);
  MockHandshakerClient* mock_client =
      reinterpret_cast<MockHandshakerClient*>(client);
  return mock_client->ClientStart();
}

static tsi_result mock_server_start(void* client, grpc_slice* bytes_received) {
  GPR_ASSERT(client != nullptr);
  MockHandshakerClient* mock_client =
      reinterpret_cast<MockHandshakerClient*>(client);
  return mock_client->ServerStart(bytes_received);
}
...
static tsi_result s2a_create_mock_handshaker_client(
  ...
  *client = new MockHandshakerClient(handshaker, channel, interested_parties,
  ...
  (*client)->set_mock_client_start_for_testing(mock_client_start);
  (*client)->set_mock_server_start_for_testing(mock_server_start);
...
TEST_F(S2AMockHandshakeTest, CheckHandshakerNextFailure) {
  ...
  /** Server start. **/
  {
    grpc_core::ExecCtx exec_ctx;
    EXPECT_EQ(
        tsi_handshaker_next(server_handshaker_, /*received_bytes=*/nullptr,
                            /*received_bytes_size=*/0,
                            /*bytes_to_send=*/nullptr,
                            /*bytes_to_send_size=*/nullptr,
                            /*result=*/nullptr, check_must_not_be_called,
...
TEST_F(S2AMockHandshakeTest, CheckHandleResponseNullptrHandshaker) {
  ...
  s2a_tsi_handshaker* s2a_handshaker =
      reinterpret_cast<s2a_tsi_handshaker*>(client_handshaker_);
  grpc_slice slice = grpc_empty_slice();
